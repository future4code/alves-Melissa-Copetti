...
const verificarPrimeiroNumero = (digitos: string): boolean => {
...
const verificaSegundoNumero = (digitos: string): boolean => {
...
const funcaoRetornarCpf = (cpf: string): boolean => {
  let cpfApenasNumeros = cpf.split(".").join("");
  cpfApenasNumeros = cpfApenasNumeros.replace("-", "");
  if (!mesmosDigitos(cpfApenasNumeros)) {
    if (verificarPrimeiroNumero(cpfApenasNumeros)) {
      if (verificaSegundoNumero(cpfApenasNumeros)) {
        return true;
      }
    }
  }
...
console.log(funcaoRetornarCpf("018.249.420-95"));
